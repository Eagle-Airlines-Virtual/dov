...
$database->execute('DELETE FROM acars WHERE created_at < DATE_SUB(NOW(), INTERVAL 18 HOUR)');
...
$database->execute('UPDATE ' . dbPrefix . 'acars SET status=?, lat=?, lon=?, distance=?, heading=?, gs=?, altitude=?, updated_at=NOW()  WHERE id=?', array(phaseToStatus($_POST['phase']), $_POST['latitude'], $_POST['longitude'], $_POST['distanceRemaining'], $_POST['heading'], $_POST['groundSpeed'], $_POST['altitude'],  $pirepID));
